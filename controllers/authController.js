...
const devLogin = async (req, res) => {
    try {
        const role = (req.body?.role || req.query?.role || 'admin').toLowerCase();
        const VALID_ROLES = ['admin', 'staff', 'student', 'guard', 'worker'];
        const targetRole = VALID_ROLES.includes(role) ? role : 'admin';

        // Find the first user of this role
        const { rows } = await db.query(
            'SELECT * FROM users WHERE LOWER(role) = $1 LIMIT 1',
            [targetRole]
        );
        
        const userId = rows.length > 0 ? rows[0].id : 0;
        ...
        return res.json({ message: 'Dev bypass successful', token, role: targetRole });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Server error in dev login' });
    }
};

module.exports = { login, verifyOtp, changePassword, devLogin };
